...
nvim_setup_clean()
{
    if [[ -f ${nvim_setup_status_file} ]]; then
        nvim_setup_parse_prev_installation_vars \
            nvim_config_dir nvim_build_dir nvim_src_dir
    fi

    nvim_setup_info "Cleaning all generated files / directories"
    nvim_setup_exec_cmd "rm -rf ${nvim_config_dir} ${nvim_build_dir} ${nvim_src_dir} ${nvim_shada_dir_default} ~/ccls"
    ...
    local _shellrc
    nvim_setup_get_shellrc _shellrc
    if nvim_setup_exec_cmd "grep \"^# Add [A-Z-a-z]\+\" ${_shellrc}"; then
...
nvim_setup_parse_opts()
{
    ...
    _args="$@"
    _opts=$(getopt -o b:c:Cds:vh --long build: --long config: --long clean --long dry-run --long source: --long help --long verbose -- ${_args})
    [ $? -ne 0 ] && return 1
    eval set -- ${_opts}
    while true; do
        case $1 in
